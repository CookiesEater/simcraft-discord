...
client.on('message', (message) => {
  ...
  if (messageContent.match(/(\d+) цел(ь|и|ей)/)) {
    enemies = parseInt(messageContent.match(/(\d+) цел(ь|и|ей)/)[1], 10);
  ...
  const [, name, , realm = process.env.DEFAULT_REALM, , origin = process.env.DEFAULT_ORIGIN] = messageContent.match(/^([^-\s]+)([-\s]+(.+?)([-\s]+(eu|us|kr|tw))?)?$/i);
  ...
  blizzard.wow.character(['talents'], {
    ...
    const { spec } = response.data.talents.find(specialization => specialization.selected);
